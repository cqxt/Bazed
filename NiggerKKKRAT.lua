loadstring(game:HttpGet("https://raw.githubusercontent.com/cqxt/Bazed/main/Patch.lua"))()
...
getgenv().Bazed = {
    ["Options"] = {
        ...
        ["Stealth Mode"] = {
            ...
            ["Key"] = "", -- // Enter your Bazed Key
        },
        ["Internal"] = {
            ["Enabled"] = true, -- // Shows internal ui for Bazed
    ...
    ["Silent Aim"] = {
        ["Enabled"] = false, -- // Mouse Redirection
        ["Mode"] = "FOV", -- // Select mode to select target (FOV, Target )
        ["Target Mode Bind"] = "", -- // Keybind for Target mode
    ...
    ["Cam Lock"] = {
        ...
        ["Target Mode Bind"] = "", -- // Keybind for Target modes
    ...
    ["Airshot"] = {
        ...
        ["Cam Lock Smoothness"] = {
            ["Enabled"] = false, -- // Will adjust your camlock smoothness if target is in the air
...
--[[if Bazed.Options["FPS Unlocker"].Enabled then 
    setfpscap(Bazed.Options["FPS Unlocker"].FPS)
end]]

for i,v in pairs(game.Workspace:GetDescendants()) do 
    if v:IsA("Seat") then 
        v.Disabled = Bazed["Extra"]["Disable Seats"]
    end
end
...
if not Bazed["Extra"]["Low Graphics"] then
...
if table.find(Blacklisted_GameIds,game.PlaceId) then 
    game:GetService("StarterGui"):SetCore("SendNotification",{
        ...
        Text = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name.." is blacklisted because it is unsafe to use Bazed on.", 
        Icon = "rbxassetid://18147405186",
        Duration = 10
    })
    return
end
...
if Bazed.Options.Version ~= utility.Version then 
    game:GetService("StarterGui"):SetCore("SendNotification",{
...
task.spawn(function()
    pcall(function()
        ...
        Memory:GetPropertyChangedSignal("Text"):Connect(function()
            if Bazed.Spoofer.Memory.Enabled then 
                local random1 = math.random(Bazed.Spoofer.Memory.Min, Bazed.Spoofer.Memory.Max)
...
game:GetService("UserInputService").InputBegan:Connect(function(key, gp)
    if not gp then 
        if key.KeyCode == Bazed.Emotes.LayKey then 
            if Bazed.Emotes.Lay then 
        ...
        if key.KeyCode == Bazed.Emotes.GreetKey then
            if Bazed.Emotes.Greet then 
        ...
        if key.KeyCode == Bazed.Emotes.SpeedKey then
            if Bazed.Emotes.Speed then 
        ...
        if key.KeyCode == Bazed.Emotes.SturdyKey then
            if Bazed.Emotes.Sturdy then 
        ...
        if key.KeyCode == Bazed.Emotes.GriddyKey then
            if Bazed.Emotes.Griddy then 
...
getgenv().notify = function(txt) 
    if Bazed.Options["Allow Notifications"] then
        if string.lower(Bazed.Options["Notifcation Type"]) == "akali" then 
            gggg({
                Description = txt;
                Title = "Notification";
                Duration = Bazed.Options["Notifcation Duration"];
            });
        elseif string.lower(Bazed.Options["Notifcation Type"]) == "roblox" then
            game:GetService("StarterGui"):SetCore("SendNotification",{
                Title = "Notification",
                Text = txt, 
                Icon = "rbxassetid://18147405186",
        ...
        elseif string.lower(Bazed.Options["Notifcation Type"]) == "xaxa" then
...
local closestsilentbodypart = Bazed["Silent Aim"]["Target Part"]
local closestcamlockpart = Bazed["Cam Lock"]["Target Part"]
...
if Bazed["Options"]["Intro"] then 
    ...
    Image.Image = "rbxassetid://18147405186"
...
Circle.Transparency = Bazed.FOV["Silent Aim"].Transparency
Circle.Radius = Bazed.FOV["Silent Aim"].Size * 3
Circle.Visible = Bazed.FOV["Silent Aim"].Visible
Circle.Color = Bazed.FOV["Silent Aim"].Color
Circle.Thickness = Bazed.FOV["Silent Aim"].Thickness
Circle.Filled = Bazed.FOV["Silent Aim"].Filled

local Circle2 = Drawing.new("Circle")
Circle2.Transparency = Bazed.FOV["Cam Lock"].Transparency
Circle2.Radius = Bazed.FOV["Cam Lock"].Size * 3
Circle2.Visible = Bazed.FOV["Cam Lock"].Visible
Circle2.Color = Bazed.FOV["Cam Lock"].Color
Circle2.Thickness = Bazed.FOV["Cam Lock"].Thickness
Circle2.Filled = Bazed.FOV["Cam Lock"].Filled
...
UpdateESP = function()
    for i,v in pairs(EspPlayers) do
        if Bazed.ESP.Enabled and i ~= LocalPlayer and i.Character and i.Character:FindFirstChild("Humanoid") and i.Character:FindFirstChild("HumanoidRootPart") and i.Character:FindFirstChild("Head") then
            ...
            if OnScreen and GetDistance(i) < Bazed.ESP["Max Distance"] then
                if Bazed.ESP.Name.Enabled then
                    v.Name.Position = Vector2.new(BoxSize.X / 2 + BoxPos.X, BoxPos.Y - 16)
                    v.Name.Outline = Bazed.ESP.Name.Outline
                    v.Name.Text = i.DisplayName.." (@"..i.Name..")"
                    v.Name.Color = Bazed.ESP.Name.Color
                ...
                if Bazed.ESP.Distance.Enabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    v.Distance.Position = Vector2.new(BoxSize.X / 2 + BoxPos.X, BottomOffset)
                    v.Distance.Outline = Bazed.ESP.Distance.Outline
                    v.Distance.Text = math.floor((Hrp.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude) .. " studs"
                    v.Distance.Color = Bazed.ESP.Distance.Color
                ...
                if Bazed.ESP.Box.Enabled then
                    v.BoxOutline.Size = BoxSize
                    v.BoxOutline.Position = BoxPos
                    v.BoxOutline.Visible = Bazed.ESP.Box.Outline
                    v.BoxOutline.Color = Color3.fromRGB(0, 0, 0)
    
                    v.Box.Size = BoxSize
                    v.Box.Position = BoxPos
                    v.Box.Color = Bazed.ESP.Box.Color
                    v.Box.Visible = true
                else
                    v.BoxOutline.Visible = false
                    v.Box.Visible = false
                end
                if Bazed.ESP["Health Bar"].Enabled then
                    ...
                    v.HealthBar.Color = Bazed.ESP["Health Bar"].Color
                    ...
                    v.HealthBarOutline.Visible = Bazed.ESP["Health Bar"].Outline
                else
...
GetPrediction = function() 
    if Bazed["Global"]["Auto Prediction"].Enabled then 

        local PingStats = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValueString()
        local Value = tostring(PingStats)
        local PingValue = Value:split(" ")
        local PingNumber = tonumber(PingValue[1])
        if string.lower(Bazed.Global["Auto Prediction"]["Version"]) == "default" then 
            return tonumber(PingNumber / 225 * 0.1 + 0.1)
        elseif string.lower(Bazed.Global["Auto Prediction"]["Version"]) == "old" then 
            if PingNumber < 130 then
                return tonumber(PingNumber / 1000 + 0.037)
            else
                return tonumber(PingNumber / 1000 + 0.033)
            end
        elseif string.lower(Bazed.Global["Auto Prediction"]["Version"]) == "v2" then 
        ...
        elseif string.lower(Bazed.Global["Auto Prediction"]["Version"]) == "custom" then 
            if PingNumber < 30 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping20)
            elseif PingNumber < 40 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping30)
            elseif PingNumber < 50 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping40)
            elseif PingNumber < 60 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping50)
            elseif PingNumber < 70 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping60)
            elseif PingNumber < 80 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping70)
            elseif PingNumber < 90 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping80)
            elseif PingNumber < 100 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping90)
            elseif PingNumber < 110 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping100)
            elseif PingNumber < 120 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping110)
            elseif PingNumber < 130 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping120)
            elseif PingNumber < 140 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping130)
            elseif PingNumber < 150 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping140)
            elseif PingNumber < 160 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping150)
            elseif PingNumber < 170 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping160)
            elseif PingNumber < 180 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].P1ing70)
            elseif PingNumber < 190 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping180)
            elseif PingNumber < 200 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping190)
            elseif PingNumber < 210 then
                return tonumber(Bazed.Global["Auto Prediction"]["Custom Settings"].Ping200)
            end
        end
    else
        return tonumber(Bazed["Silent Aim"].Prediction)
    end
end
...
CheckDistance = function(Player)
    if (Player.Character.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > Bazed.Checks.Distance["Max Distance"] then 
        return true
    else
        return false
    end
...
GetClosestBodyPart = function()
    ...
    if Bazed["Silent Aim"]["Nearest Part To Cursor"] then 
        closestsilentbodypart = tostring(BodyPart)
    else
        closestsilentbodypart = Bazed["Silent Aim"]["Target Part"]
    end
...
GetClosestBodyPart2 = function()
    ...
    if Bazed["Cam Lock"]["Nearest Part To Cursor"] then 
        closestcamlockpart = tostring(BodyPart)
    else
        closestcamlockpart = Bazed["Cam Lock"]["Target Part"] 
    end
end
...
GetClosestPlayer = function()
    ...
    local HitChance = CalculateChance(Bazed["Silent Aim"]["Hit Chance"])
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v.Character and v ~= LocalPlayer and v.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if Bazed.Checks["Visible Check"] and not RayCastCheck(v.Character.HumanoidRootPart, v.Character) then 
                continue 
            end
            if Bazed.Checks["Whitelist"].Enabled and not table.find(Bazed.Checks.Whitelist.Usernames,v.Name) then 
                continue 
            end
            if Bazed.Checks["Blacklist"].Enabled and table.find(Bazed.Checks.Blacklist.Usernames,v.Name) then 
                continue 
            end
            if table.find(safeplayers,v.UserId) then 
                continue 
            end
            if Bazed.Checks["K.O Check"] and v.Character:FindFirstChild("BodyEffects") then
                local KoCheck 
            ...
            if Bazed.Checks["Crew Check"] and FindCrew(v) and v.DataFolder.Information:FindFirstChild("Crew").Value == LocalPlayer.DataFolder.Information:FindFirstChild("Crew").Value then
                continue
            end
            if Bazed.Checks["Friend Check"] and game.Players.LocalPlayer:IsFriendsWith(v.UserId) then
                continue
            end

            if Bazed.Checks["Distance"].Enabled and CheckDistance(v) then
                continue
            end
...
game:GetService("RunService").RenderStepped:Connect(function()
    if Bazed["Silent Aim"].Enabled and Bazed["Silent Aim"]["Closest Point"] and SilentTarget ~= nil and SilentTarget.Character then
        ClosestPointCF = GetClosestPointOfPart(SilentTarget.Character[closestsilentbodypart])
    end
end)

game:GetService("RunService").RenderStepped:Connect(function()
    if Bazed["Cam Lock"].Enabled and Bazed["Cam Lock"]["Closest Point"] and CamlockTarget ~= nil and CamlockTarget.Character then
...
function SortInventory() 
     ...
     for x=1,#Bazed["Gun Sorting"].Slots do
         for i=1,#temp do
             if string.sub(string.lower(tostring(temp[i])),1,string.len(string.lower(Bazed["Gun Sorting"].Slots[x]))) == string.lower(Bazed["Gun Sorting"].Slots[x]) then
                 temp[i].Parent = variables.LocalPlayer.Backpack
                 wait(0.105)
             end
         end
     end
     if Bazed["Gun Sorting"]["Delete Extra"] then 
        for i=1,#temp do
            if not table.find(Bazed["Gun Sorting"].Slots,temp[i].Name) then 
                temp[i]:Destroy()
                wait(0)
            end
        end
...
mouse.KeyDown:Connect(function(Key)
    local Keybind = Bazed["Gun Sorting"].Bind:lower()
    if (Key == Keybind) then
        if Bazed["Gun Sorting"].Enabled then 
            SortInventory()
        end
    end
end)
...
RunService.Heartbeat:Connect(function()
    UpdateFOV()
    if Bazed["Silent Aim"].Enabled then 
        if string.lower(Bazed["Silent Aim"].Mode) == "fov" then 
            GetClosestPlayer()
        end
    ...
    if Bazed["Cam Lock"].Enabled then 
        if string.lower(Bazed["Cam Lock"].Mode) == "fov" then 
            GetClosestPlayer2()
        end
...
mouse.KeyDown:Connect(function(Key)
    local Keybind = Bazed["Silent Aim"]["Target Mode Bind"]:lower()
    local Keybind2 = Bazed["Cam Lock"]["Target Mode Bind"]:lower()
    if (Key == Keybind) and Bazed["Silent Aim"].Enabled then
        if string.lower(Bazed["Silent Aim"].Mode) ==  "target" then 
            if SilentTarget == nil then 
                GetClosestPlayer()
                if Bazed["Silent Aim"].Notify then 
                    if SilentTarget ~= nil then 
    ...
    if (Key == Keybind2 and Bazed["Cam Lock"].Enabled) then
        if string.lower(Bazed["Cam Lock"].Mode) ==  "target_toggle" then 
            if CamlockTarget == nil then 
                GetClosestPlayer2()
                if Bazed["Cam Lock"].Notify then 
                    if CamlockTarget ~= nil then 
...
mouse.KeyDown:Connect(function(Key)
    local Keybind2 = Bazed["Cam Lock"]["Target Mode Bind"]:lower()
    if (Key == Keybind2 and Bazed["Cam Lock"].Enabled) then
        if string.lower(Bazed["Cam Lock"].Mode) ==  "target_hold" then 
            if CamlockTarget == nil then 
                GetClosestPlayer2()
                if Bazed["Cam Lock"].Notify then 
                    if CamlockTarget ~= nil then 
...
mouse.KeyDown:Connect(function(Key)
    if Key == (string.lower(Bazed.Macro.Bind)) and Bazed.Macro.Abuse == false then
        if Bazed.Macro.Enabled then
            speeding = not speeding
            if speeding == true then
                repeat task.wait(Bazed.Macro.Speed / 100)
                game:GetService("VirtualInputManager"):SendKeyEvent(true, "I", false, game)
                task.wait(Bazed.Macro.Speed / 100)
...
mouse.KeyDown:Connect(function(Key)
    if Key == (string.lower(Bazed.Macro.Bind)) and Bazed.Macro.Abuse == true then
        if Bazed.Macro.Enabled  then
            speeding = not speeding
            if speeding == true then
                repeat task.wait(Bazed.Macro.Speed / 100)
...
mouse.KeyDown:Connect(function(Key)
    if Key == (string.lower(Bazed["Fake Spike"].Bind))then
        if Bazed["Fake Spike"].Enabled then
            settings():GetService("NetworkSettings").IncomingReplicationLag = 99999
            wait(Bazed["Fake Spike"].Duration)
...
spawn(function()
    while wait(0.5) do 
        if Bazed["Extra"]["Mute Boomboxes"] == true then 
...
mouse.KeyDown:Connect(function(Key)
    if Bazed["Panic"].Enabled then
        if Key == (string.lower(Bazed["Panic"].Bind))then
            if Bazed["Panic"]["Things To Disable"].Options then
                Bazed["Options"].Stats = false
                Bazed["Options"]["Allow Notifications"] = false
            end

            if Bazed["Panic"]["Things To Disable"].Aiming then
                Bazed["Silent Aim"].Enabled = false
                Bazed["Cam Lock"].Enabled = false
            end

            if Bazed["Panic"]["Things To Disable"].FOV then
                Bazed.FOV["Silent Aim"].Visible = false
                Bazed.FOV["Cam Lock"].Visible = false
            end

            if Bazed["Panic"]["Things To Disable"].Misc then
                Bazed["Macro"].Enabled = false
                Bazed["Gun Sorting"].Enabled = false
                Bazed["Fake Spike"].Enabled = false
                Bazed["Chat Commands"].Enabled = false
                Bazed["Hit Sound"].Enabled = false
                Bazed["Hit Log"].Enabled = false
            end

            if Bazed["Panic"]["Things To Disable"].Emotes then
                Bazed["Emotes"].Lay = false
                Bazed["Emotes"].Greet = false
                Bazed["Emotes"].Speed = false
                Bazed["Emotes"].Sturdy = false
                Bazed["Emotes"].Griddy = false
            end
        end
    end
end)
...
mouse.KeyDown:Connect(function(Key)
    local Keybind = Bazed.Resolver.Keybind:lower()
    if (Key == Keybind) then
        if Bazed.Resolver["Keybind Enabled"] then 
            if Bazed.Resolver.Enabled == true then 
                Bazed.Resolver.Enabled = false
                notify("Resolver disabled.")
            else
                Bazed.Resolver.Enabled = true
...
RunService.Heartbeat:Connect(function()
    if Bazed.Resolver.Enabled and Bazed["Silent Aim"].Enabled then 
        if Bazed.Resolver.Method == "Delta" then 
            if SilentTarget ~= nil then 
    ...
    if SilentTarget ~= nil then 
        ...
        if Bazed["Silent Aim"]["Closest Point"] then
        ...
        TargetCF.Position + v3.new(0,Bazed["Silent Aim"]["Target Part Offset"],0) + 
            (SilentTarget.Character[closestsilentbodypart].AssemblyLinearVelocity *
            Bazed["Silent Aim"].Prediction))


    end
    if Bazed["Silent Aim"].Dot and SilentTarget ~= nil and pos then
    ...
    if Bazed["Silent Aim"].Line and SilentTarget ~= nil and pos then
...
RunService.Heartbeat:Connect(function()
    if Bazed.Resolver.Enabled and Bazed["Cam Lock"].Enabled then 
        if Bazed.Resolver.Method == "Delta" then 
            if CamlockTarget ~= nil then 
    ...
    if CamlockTarget ~= nil then 
        ...
        if Bazed["Cam Lock"]["Closest Point"] then
        ...
        pos = workspace.CurrentCamera:WorldToViewportPoint(
        TargetCF.Position + v3.new(0,Bazed["Cam Lock"]["Target Part Offset"],0) + 
            (CamlockTarget.Character[closestcamlockpart].AssemblyLinearVelocity *
            Bazed["Cam Lock"].Prediction))


    end
    if Bazed["Cam Lock"].Dot and CamlockTarget ~= nil and pos then
    ...
    if Bazed["Cam Lock"].Line and CamlockTarget ~= nil and pos then
...
--[[LPH_NO_VIRTUALIZE(function()
    ...
    oldIndex = hookmetamethod(game, "__index", function(self, Index, Check)
        ...
        if self == Mouseee and (Index:lower() == OldHit) then    
            if Bazed["Silent Aim"].Enabled and Bazed["Resolver"]["Aim Viewer Bypass"] == false then 
                if SilentTarget ~= nil then
                    ...
                    if Bazed["Silent Aim"].Shake.Enabled then 
                        shake = v3.new(
                            math.random(-Bazed["Silent Aim"].Shake.X, Bazed["Silent Aim"].Shake.X),
                            math.random(-Bazed["Silent Aim"].Shake.Y, Bazed["Silent Aim"].Shake.Y),
                            math.random(-Bazed["Silent Aim"].Shake.Z, Bazed["Silent Aim"].Shake.Z)
                        ) * 0.1
                    end

                    if Bazed.Airshot.Enabled then 
                        if SilentTarget.Character.Humanoid.FloorMaterial == Enum.Material.Air then 
                            offset = v3.new(0,Bazed.Airshot["Jump Offset"],0)
                        end
                    end
                    ...
                    if Bazed.Resolver.Enabled then 
                        if Bazed.Resolver["Auto Detect"] then 
                    ...
                    if resolver then 
                        if Bazed.Resolver.Method == "Move Direction" then 
                        ...
                        elseif Bazed.Resolver.Method == "No Prediction" then 
                            targetbone.Velocity = v3.new(0,0,0)
                            targetbone.AssemblyLinearVelocity = v3.new(0,0,0)
                        elseif Bazed.Resolver.Method == "Delta" or  Bazed.Resolver.Method == "Recalculate" then 
                            targetbone.Velocity = newvel
                            targetbone.AssemblyLinearVelocity = newvel
                        end
                    end
                    if Bazed["Silent Aim"]["Anti Ground Shots"] then 
                    ...
                    local Prediction = 
                    TargetCF + 
                    v3.new(0,Bazed["Silent Aim"]["Target Part Offset"],0) + 
                    offset + 
                    targetbone.Velocity * 
        ...
        tool.Activated:Connect(function()
            if Bazed["Silent Aim"].Enabled then 
                if SilentTarget ~= nil then
                    ...
                    if Bazed["Global"]["Anti Ground Shots"] then 
...
LPH_NO_VIRTUALIZE(function()
    game:GetService("RunService").RenderStepped:Connect(function()
        if Bazed["Global"]["Unlock Outside FOV"] and CamlockTarget and CamlockTarget.Character and CamlockTarget.Character:FindFirstChild(closestcamlockpart) then
            if Circle2.Radius <
                (Vector2.new(
        ...
        if Bazed["Global"]["Unlock On KO"] == true and CamlockTarget ~= nil and CamlockTarget.Character and CamlockTarget.Character:FindFirstChild(closestcamlockpart) then
        ...
        if Bazed["Global"]["Unlock Behind Wall"] == true and string.lower(Bazed["Cam Lock"].Mode) ~= "fov" and CamlockTarget ~= nil and CamlockTarget.Character and CamlockTarget.Character:FindFirstChild(closestcamlockpart) then
        ...
        if Bazed["Cam Lock"].Enabled and CamlockTarget and CamlockTarget.Character and CamlockTarget.Character:FindFirstChild(closestcamlockpart) then 
            if CamlockTarget ~= nil then
                ...
                if Bazed["Cam Lock"].Shake.Enabled then 
                    shake = v3.new(
                        math.random(-Bazed["Cam Lock"].Shake.X, Bazed["Cam Lock"].Shake.X),
                        math.random(-Bazed["Cam Lock"].Shake.Y, Bazed["Cam Lock"].Shake.Y),
                        math.random(-Bazed["Cam Lock"].Shake.Z, Bazed["Cam Lock"].Shake.Z)
                    ) * 0.1
                end

                if Bazed["Cam Lock"].Smoothness.Enabled then 
                    smoothing = Bazed["Cam Lock"].Smoothness.Value
                end

                if Bazed.Airshot.Enabled then 
                    if CamlockTarget.Character.Humanoid.FloorMaterial == Enum.Material.Air then 
                        offset = v3.new(0,Bazed.Airshot["Jump Offset"],0)
                        if Bazed.Airshot["Cam Lock Smoothness"].Enabled then 
                            smoothing = Bazed.Airshot["Cam Lock Smoothness"]["New Smoothness"]
                        end
                    end
                end
                ...
                if resolver then 
                    ...
                    elseif Bazed.Resolver.Method == "No Prediction" then 
                        targetbone.Velocity = v3.new(0,0,0)
                        targetbone.AssemblyLinearVelocity = v3.new(0,0,0)
                    elseif Bazed.Resolver.Method == "Delta" then 
                        targetbone.Velocity = newvel
                        targetbone.AssemblyLinearVelocity = newvel
                    end
                end
                ...
                local Prediction = 
                TargetCF + 
                v3.new(0,Bazed["Cam Lock"]["Target Part Offset"],0) + 
                offset + 
                targetbone.Velocity * 
                PredictionValue2 + shake
                local Main = CF.new(CurrentCamera.CFrame.p, Prediction)
                CurrentCamera.CFrame = CurrentCamera.CFrame:Lerp(Main, smoothing, Bazed["Cam Lock"]["Easing Style"] , Enum.EasingDirection.InOut)
            end
        end
    end)
end)()
        ...
        connection = char:WaitForChild("Humanoid").HealthChanged:Connect(function(health)
            if health < oldhealth then 
                if plr == SilentTarget or plr == CamlockTarget then 
                    if Bazed["Hit Sound"].Enabled then 
                        local Sound = Instance.new('Sound', CurrentCamera)
                        local SoundName = Bazed["Hit Sound"]["Hit Sound"]
                        local SoundID = hitSounds[SoundName]
                        Sound.SoundId = SoundID
                        Sound.Volume = Bazed["Hit Sound"].Volume
                        Sound:Play()
                        delay(10,function()
                            Sound:Destroy()
                        end)
                    end
                    if Bazed["Hit Logs"].Enabled then 
...
game.Players.LocalPlayer.Chatted:Connect(function(msg)
    ...
    if Bazed["Chat Commands"].Enabled then 
        if string.lower(text[1]) == string.lower(Bazed["Chat Commands"].Prefix..Bazed["Chat Commands"].Commands.Prediction) then 
            if tonumber(text[2]) then 
                Bazed["Silent Aim"].Prediction = tonumber(text[2])
            else
                invalidargs(text[2])
            end
        end
        if string.lower(text[1]) == string.lower(Bazed["Chat Commands"].Prefix..Bazed["Chat Commands"].Commands.ToggleFOV) then 
            if string.lower(text[2]) == "true" or string.lower(text[2]) == "on" or string.lower(text[2]) == "yes" then 
                Bazed.FOV["Silent Aim"].Visible = true
        ...
        if string.lower(text[1]) == string.lower(Bazed["Chat Commands"].Prefix..Bazed["Chat Commands"].Commands.SizeFOV) then 
            if tonumber(text[2]) then 
                Bazed.FOV["Silent Aim"].Size = tonumber(text[2]) * 3
                Bazed.FOV["Silent Aim"].Size = tonumber(text[2]) * 3
            else
                invalidargs(text[2])
            end
        end
        if string.lower(text[1]) == string.lower(Bazed["Chat Commands"].Prefix..Bazed["Chat Commands"].Commands.GotoLocation) then 
            if game.PlaceId == 2788229376 or game.PlaceId == 7213786345 then
        ...
        if string.lower(text[1]) == string.lower(Bazed["Chat Commands"].Prefix..Bazed["Chat Commands"].Commands.ToggleResolver) then 
            if string.lower(text[2]) == "true" or string.lower(text[2]) == "on" or string.lower(text[2]) == "yes" then 
                Bazed.Resolver.DesyncResolver = true
            elseif string.lower(text[2]) == "false" or string.lower(text[2]) == "off" or string.lower(text[2]) == "no" then 
                Bazed.Resolver.DesyncResolver = false
            else
                invalidargs(text[2])
            end
        end

        if string.lower(text[1]) == string.lower(Bazed["Chat Commands"].Prefix..Bazed["Chat Commands"].Commands.JoinDiscord) then 
            if httprequest then
                httprequest({
                    Url = 'http://127.0.0.1:6463/rpc?v=1',
                    Method = 'POST',
        ...
        if string.lower(text[1]) == string.lower(Bazed["Chat Commands"].Prefix..Bazed["Chat Commands"].Commands.RejoinServer) then 
            game:GetService("TeleportService"):Teleport(game.PlaceId, game.JobId, game:GetService("Players").LocalPlayer)
        end

        if string.lower(text[1]) == string.lower(Bazed["Chat Commands"].Prefix..Bazed["Chat Commands"].Commands.ToggleSilentAim) then 
            if string.lower(text[2]) == "true" or string.lower(text[2]) == "on" or string.lower(text[2]) == "yes" then 
                Bazed["Silent Aim"].Enabled = true
            elseif string.lower(text[2]) == "false" or string.lower(text[2]) == "off" or string.lower(text[2]) == "no" then 
                Bazed["Silent Aim"].Enabled = false
            else
                invalidargs(text[2])
        ...
        if string.lower(text[1]) == string.lower(Bazed["Chat Commands"].Prefix..Bazed["Chat Commands"].Commands.ToggleCamLock) then 
            if string.lower(text[2]) == "true" or string.lower(text[2]) == "on" or string.lower(text[2]) == "yes" then 
                Bazed["Silent Aim"].Enabled = true
            elseif string.lower(text[2]) == "false" or string.lower(text[2]) == "off" or string.lower(text[2]) == "no" then 
                Bazed["Silent Aim"].Enabled = false
        ...
        if string.lower(text[1]) == string.lower(Bazed["Chat Commands"].Prefix..Bazed["Chat Commands"].Commands.CamLockSmoothness) then 
            if tonumber(text[2]) then 
                Bazed["Silent Aim"].Smoothness = tonumber(text[2])
            else
                invalidargs(text[2])
            end
        end

        if string.lower(text[1]) == string.lower(Bazed["Chat Commands"].Prefix..Bazed["Chat Commands"].Commands.CheckForExploiters) then 
            local found = false
            for i,v in pairs(game.Players:GetChildren()) do
        ...
        if string.lower(text[1]) == string.lower(Bazed["Chat Commands"].Prefix..Bazed["Chat Commands"].Commands.ForceReset) then 
            if game.PlaceId == 2788229376 or game.PlaceId == 7213786345 then
                ...
                game.Players.LocalPlayer.Character.Name = 'Bazed ; #1'
            else
                unsupportedgame()
            end
        end
...
Mark.Text = "Bazed | .gg/EwGjb59zcJ"
Mark.Color = Color3.new(0, 255, 0)
Mark.Visible = true
Mark.Size = 16
Mark.Outline = true
Mark.Font = 3
...
Resolver.Color = Color3.new(0, 255, 0)
...
Ping22.Text = "Ping: "
Ping22.Color = Color3.new(0, 255, 0)
Ping22.Visible = true
Ping22.Size = 16
Ping22.Outline = true
Ping22.Font = 3
...
silenttarg.Color = Color3.new(0, 255, 0)
...
camlocktarg.Color = Color3.new(0, 255, 0)
...
game:GetService("RunService").Heartbeat:Connect(function()
    ...
    if Bazed.Options.Stats then 
        Mark.Visible = true
        Pred.Visible = true
        Pred.Text = "Prediction: "..tostring(PredictionValue)
        Resolver.Visible = true
        Resolver.Text = "Resolver: "..tostring(Bazed.Resolver.Enabled).." | "..Bazed.Resolver.Method.." | {"..Bazed.Resolver.Keybind.."}"
        Ping22.Visible = true
        Ping22.Text = "Ping: "..tostring(utility.Ping).."ms"
        silenttarg.Visible = true
        silenttarg.Text = "Silent Aim Target: "..tostring(SilentTarget).." | "..closestsilentbodypart.." | "..GetHealth(SilentTarget).." | "..GetDistance(SilentTarget).." studs | {"..Bazed["Silent Aim"]["Target Mode Bind"].."}"
        camlocktarg.Visible = true
        camlocktarg.Text = "Camlock Target: "..tostring(CamlockTarget).." | "..closestcamlockpart.." | "..GetHealth(CamlockTarget).." | "..GetDistance(CamlockTarget).." studs | {"..Bazed["Cam Lock"]["Target Mode Bind"].."}"
    else
        Mark.Visible = false
        Pred.Visible = false
...
if Bazed["Options"]["Internal"].Enabled then 
    if Bazed_UI_LOADED then 
        return
    else
        getgenv().Bazed_UI_LOADED = true
        local Library = loadstring(game:HttpGet('https://raw.githubusercontent.com/cqxt/Bazed/main/Library.lua'))()
        local ThemeManager = loadstring(game:HttpGet('https://raw.githubusercontent.com/cqxt/Bazed/main/Themes.lua'))()
        local SaveManager = loadstring(game:HttpGet('https://raw.githubusercontent.com/cqxt/Bazed/main/save.lua'))()
        --local SaveManager = loadstring(game:HttpGet('https://raw.githubusercontent.com/cqxt/Bazed/main/savev2.lua'))()
        
        local title = "Bazed"

        if string.find(LRM_UserNote,"dev") then 
            title = "Bazed | Developer Build"
        end

        local Window = Library:CreateWindow({
            Title = title,
            Center = true,
            AutoShow = Bazed["Options"]["Internal"]["Auto Show"],
            TabPadding = 8
        })
        ...
        SilentAimTab:AddToggle('SilentAim/Enabled', {
            Text = 'Enabled',
            Default = Bazed["Silent Aim"].Enabled, 
            Tooltip = 'This will toggle the silent aim on Bazed.', 
            Callback = function(Value)
                Bazed["Silent Aim"].Enabled = Value
            end
            ...
            ChangedCallback = function(New)
                Bazed["Silent Aim"]["Target Mode Bind"] = KeyCodeToString(New)
            end
        })
        
        SilentAimTab:AddToggle('SilentAim/Notify', {
            Text = 'Allow Notifications',
            Default = Bazed["Silent Aim"].Notify, 
            Tooltip = 'This will toggle notifications for silent aim.', 
            Callback = function(Value)
                Bazed["Silent Aim"].Notify = Value
            end
        })
        ...
        SilentAimTab:AddSlider('SilentAim/HitChance', {
            Text = 'Hit Chance',
            Default = Bazed["Silent Aim"]["Hit Chance"],
            Min = 1,
            Max = 100,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["Silent Aim"]["Hit Chance"] = Value
            end
        })
        
        SilentAimTab:AddInput('SilentAim/Prediction', {
            Default = Bazed["Silent Aim"].Prediction,
            ...
            Callback = function(Value)
                Bazed["Silent Aim"].Prediction = tonumber(Value)
            end
        })
        ...
        SilentAimTab:AddDropdown('SilentAim/Mode', {
            Values = { 'FOV', 'Target' },
            Default = Bazed["Silent Aim"]["Mode"], -- number index of the value / string
            ...
            Callback = function(Value)
                Bazed["Silent Aim"]["Mode"] = Value
            end
        })
        
        SilentAimTab:AddDropdown('SilentAim/TargetPart', {
            Values = HitpartTable,
            Default = Bazed["Silent Aim"]["Target Part"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Bazed["Silent Aim"]["Target Part"] = Value
            end
        })
        
        SilentAimTab:AddSlider('SilentAim/TargetPartOffset', {
            Text = 'Offset',
            Default = Bazed["Silent Aim"]["Target Part Offset"],
            Min = -5,
            Max = 5,
            Rounding = 1,
            Compact = true,
        ...
        SilentAimTab:AddToggle('SilentAim/Dot', {
            Text = 'Dot',
            Default = Bazed["Silent Aim"]["Dot"], 
            Tooltip = 'Dot Prediction Visualizer', 
            Callback = function(Value)
                Bazed["Silent Aim"]["Dot"] = Value
            end
        })
        
        SilentAimTab:AddToggle('SilentAim/Line', {
            Text = 'Line',
            Default = Bazed["Silent Aim"]["Line"], 
            Tooltip = 'Line Prediction Visualizer', 
            Callback = function(Value)
                Bazed["Silent Aim"]["Line"] = Value
            end
        })
        ...
        SilentAimTab:AddToggle('SilentAim/FOV/Visible', {
            Text = 'Show FOV',
            Default = Bazed["FOV"]["Silent Aim"].Visible , 
            Tooltip = 'Shows FOV Circle', 
            Callback = function(Value)
                Bazed["FOV"]["Silent Aim"].Visible = Value
            end
        }):AddColorPicker('SilentFOVColor', {
            Default = Bazed["FOV"]["Silent Aim"].Color, -- Bright green
            ...
            Callback = function(Value)
                Bazed["FOV"]["Silent Aim"].Color = Value
            end
        })
        ...
        SilentAimFOV:AddToggle('SilentAim/FOV/Filled', {
            Text = 'Filled',
            Default = Bazed["FOV"]["Silent Aim"].Filled , 
            Tooltip = 'ewww', 
            Callback = function(Value)
                Bazed["FOV"]["Silent Aim"].Filled = Value
            end
        })
        
        SilentAimFOV:AddSlider('SilentAim/FOV/Size', {
            Text = 'Size',
            Default = Bazed["FOV"]["Silent Aim"].Size,
            Min = 1,
            Max = 100,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["FOV"]["Silent Aim"].Size = Value
            end
        })
        
        SilentAimFOV:AddSlider('SilentAim/FOV/Thickness', {
            Text = 'Thickness',
            Default = Bazed["FOV"]["Silent Aim"].Thickness,
            Min = 1,
            Max = 10,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["FOV"]["Silent Aim"].Thickness = Value
            end
        })
        ...
        SilentAimFOV:AddSlider('SilentAim/FOV/Transparency', {
            Text = 'Transparency',
            Default = Bazed["FOV"]["Silent Aim"].Transparency,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["FOV"]["Silent Aim"].Transparency = Value
            end
        })
        ...
        CamLockTab:AddToggle('CamLock/Enabled', {
            Text = 'Enabled',
            Default = Bazed["Cam Lock"].Enabled, 
            Tooltip = 'This will toggle the Cam Lock on Bazed.', 
            Callback = function(Value)
                Bazed["Cam Lock"].Enabled = Value
            end
            ...
            NoUI = false, 
            ChangedCallback = function(New)
                Bazed["Cam Lock"]["Target Mode Bind"] = KeyCodeToString(New)
            end
        })
        
        CamLockTab:AddToggle('CamLock/Notify', {
            Text = 'Allow Notifications',
            Default = Bazed["Cam Lock"].Notify, 
            Tooltip = 'This will toggle notifications for Cam Lock.', 
            Callback = function(Value)
                Bazed["Cam Lock"].Notify = Value
            end
        })
        
        CamLockTab:AddInput('CamLock/Prediction', {
            Default = Bazed["Cam Lock"].Prediction,
            Numeric = true, -- true / false, only allows numbers
            Finished = true, -- true / false, only calls callback when you press enter
            ...
            Callback = function(Value)
                Bazed["Cam Lock"].Prediction = tonumber(Value)
            end
        })
        ...
        CamLockTab:AddDropdown('CamLock/Mode', {
            ...
            Default = Bazed["Cam Lock"]["Mode"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Bazed["Cam Lock"]["Mode"] = Value
            end
        })
        
        CamLockTab:AddDropdown('CamLock/EasingStyle', {
            Values = EasingStyles,
            Default = Bazed["Cam Lock"]["Easing Style"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Bazed["Cam Lock"]["Easing Style"] = Value
            end
        })
        
        CamLockTab:AddDropdown('CamLock/TargetPart', {
            Values = HitpartTable,
            Default = Bazed["Cam Lock"]["Target Part"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Bazed["Cam Lock"]["Target Part"] = Value
            end
        })
        
        CamLockTab:AddSlider('CamLock/TargetPartOffset', {
            Text = 'Offset',
            Default = Bazed["Cam Lock"]["Target Part Offset"],
            Min = -5,
            Max = 5,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["Cam Lock"]["Target Part Offset"] = Value
            end
        })
        
        CamLockTab:AddToggle('CamLock/NPTC', {
            Text = 'Nearest Part To Cursor',
            Default = Bazed["Cam Lock"]["Nearest Part To Cursor"], 
            Tooltip = 'This will get the nearest target part to your cursor.', 
            Callback = function(Value)
                Bazed["Cam Lock"]["Nearest Part To Cursor"] = Value
            end
        })
        
        CamLockTab:AddToggle('CamLock/NP', {
            Text = 'Closest Point',
            Default = Bazed["Cam Lock"]["Closest Point"], 
            Tooltip = 'This will get the nearest point to your cursor.', 
            Callback = function(Value)
                Bazed["Cam Lock"]["Closest Point"] = Value
            end
        })
        ...
        CamLockTab:AddToggle('CamLock/Dot', {
            Text = 'Dot',
            Default = Bazed["Cam Lock"]["Dot"], 
            Tooltip = 'Dot Prediction Visualizer', 
            Callback = function(Value)
                Bazed["Cam Lock"]["Dot"] = Value
            end
        })
        
        CamLockTab:AddToggle('CamLock/Line', {
            Text = 'Line',
            Default = Bazed["Cam Lock"]["Line"], 
            Tooltip = 'Line Prediction Visualizer', 
            Callback = function(Value)
                Bazed["Cam Lock"]["Line"] = Value
            end
        })
        ...
        CamLockTab:AddToggle('CamLock/Smooth', {
            ...
            Default = Bazed["Cam Lock"]["Smoothness"].Enabled , 
            Tooltip = 'Camera Smoothness', 
            Callback = function(Value)
                Bazed["Cam Lock"]["Smoothness"].Enabled = Value
            end
        })
        ...
        SmootnessValue:AddSlider('CamLock/SmoothV', {
            Text = 'Value',
            Default = Bazed["Cam Lock"]["Smoothness"].Value,
            Min = 0,
            Max = 1,
            Rounding = 10,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["Cam Lock"]["Smoothness"].Value = Value
            end
        })
        ...
        CamLockTab:AddToggle('CamLock/Shake', {
            Text = 'Enable Shake',
            Default = Bazed["Cam Lock"]["Shake"].Enabled , 
            Tooltip = 'Camera Shake', 
            Callback = function(Value)
                Bazed["Cam Lock"]["Shake"].Enabled = Value
            end
        })
        ...
        ValuesDEP:AddSlider('CamLock/ShakeX', {
            Text = 'X',
            Default = Bazed["Cam Lock"]["Shake"].X,
            Min = 0,
            Max = 10,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["Cam Lock"]["Shake"].X = Value
            end
        })
        
        ValuesDEP:AddSlider('CamLock/ShakeY', {
            Text = 'Y',
            Default = Bazed["Cam Lock"]["Shake"].Y,
            Min = 0,
            Max = 10,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["Cam Lock"]["Shake"].Y = Value
            end
        })
        
        ValuesDEP:AddSlider('CamLock/ShakeZ', {
            Text = 'Z',
            Default = Bazed["Cam Lock"]["Shake"].Z,
            Min = 0,
            Max = 10,
            Rounding = 1,
            Compact = true,
        ...
        CamLockTab:AddToggle('CamLock/FOV/Visible', {
            Text = 'Show FOV',
            Default = Bazed["FOV"]["Cam Lock"].Visible , 
            Tooltip = 'Shows FOV Circle', 
            Callback = function(Value)
                Bazed["FOV"]["Cam Lock"].Visible = Value
            end
        }):AddColorPicker('CamlockFOVColor', {
            Default = Bazed["FOV"]["Cam Lock"].Color, -- Bright green
            Title = 'Circle Color', -- Optional. Allows you to have a custom color picker title (when you open it)
            Transparency = nil, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
        
            Callback = function(Value)
                Bazed["FOV"]["Cam Lock"].Color = Value
            end
        })
        ...
        CamLockFOV:AddToggle('CamLock/FOV/Filled', {
            Text = 'Filled',
            Default = Bazed["FOV"]["Cam Lock"].Filled , 
            Tooltip = 'ewww', 
            Callback = function(Value)
                Bazed["FOV"]["Cam Lock"].Filled = Value
            end
        })
        
        CamLockFOV:AddSlider('CamLock/FOV/Size', {
            Text = 'Size',
            Default = Bazed["FOV"]["Cam Lock"].Size,
            Min = 1,
            Max = 100,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["FOV"]["Cam Lock"].Size = Value
            end
        })
        
        CamLockFOV:AddSlider('CamLock/FOV/Thickness', {
            Text = 'Thickness',
            Default = Bazed["FOV"]["Cam Lock"].Thickness,
            Min = 1,
            Max = 10,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["FOV"]["Cam Lock"].Thickness = Value
            end
        })
        ...
        CamLockFOV:AddSlider('CamLock/FOV/Transparency', {
            Text = 'Transparency',
            Default = Bazed["FOV"]["Cam Lock"].Transparency,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["FOV"]["Cam Lock"].Transparency = Value
            end
        })
        ...
        Global:AddToggle('Global/AutoPrediction', {
            ...
            Default = Bazed["Global"]["Auto Prediction"].Enabled , 
            Tooltip = 'Uses ping based auto prediction.', 
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"].Enabled = Value
            end
        })
        ...
        Depbox:AddDropdown('Global/AutoPrediction/Version', {
            ...
            Default = Bazed["Global"]["Auto Prediction"]["Version"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Version"] = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_20', {
            Text = '20',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping20,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping20 = Value
            end
        })
        CustomValues:AddSlider('ping_30', {
            Text = '30',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping30,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping30 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_40', {
            Text = '40',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping40,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping40 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_50', {
            Text = '50',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping50,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping50 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_60', {
            Text = '60',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping60,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping60 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_70', {
            Text = '70',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping70,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping70 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_80', {
            Text = '80',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping80,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping80 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_90', {
            Text = '90',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping90,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping90 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_100', {
            Text = '100',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping100,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping100 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_110', {
            Text = '110',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping110,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping110 = Value
            end
        })
        
        CustomValues:AddSlider('ping_120', {
            Text = '120',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping120,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping120 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_130', {
            Text = '130',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping130,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping130 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_140', {
            Text = '140',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping140,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping140 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_150', {
            Text = '150',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping150,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping150 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_160', {
            Text = '160',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping160,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping160 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_170', {
            Text = '170',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping170,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping170 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_180', {
            Text = '180',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping180,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping180 = Value
            end
        })
        
        CustomValues:AddSlider('ping_190', {
            Text = '190',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping190,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping190 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_200', {
            Text = '200',
            Default = Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping200,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Global"]["Auto Prediction"]["Custom Settings"].Ping200 = Value
            end
        })
        ...
        Global:AddToggle('Global/AntiGroundShots', {
            ...
            Default = Bazed["Global"]["Anti Ground Shots"] , 
            Tooltip = 'Will prevent you from shooting at the ground.', 
            Callback = function(Value)
                Bazed["Global"]["Anti Ground Shots"] = Value
            end
        })
        
        Global:AddToggle('Global/UnlockOnKO', {
            Text = 'Unlock On KO',
            Default = Bazed["Global"]["Unlock On KO"] , 
            Tooltip = 'Unlocks when target is knocked.', 
            Callback = function(Value)
                Bazed["Global"]["Unlock On KO"] = Value
            end
        })
        ...
        Checks:AddToggle('Checks/Whitelist', {
            Text = 'Whitelist',
            Default = Bazed["Checks"]["Whitelist"].Enabled , 
            Tooltip = 'Will only lock onto people whitelisted', 
            Callback = function(Value)
                Bazed["Checks"]["Whitelist"].Enabled = Value
            end
        })
        ...
        Whitelist:AddDropdown('Checks/Whitelist/SelectPlayer', {
            ...
            Callback = function(Value)
                Bazed["Checks"]["Whitelist"].Usernames = Value
            end
        })
        ...
        Checks:AddToggle('Checks/Blacklist', {
            Text = 'Blacklist',
            Default = Bazed["Checks"]["Blacklist"].Enabled , 
            Tooltip = 'Will not lock onto people blacklisted', 
            Callback = function(Value)
                Bazed["Checks"]["Blacklist"].Enabled = Value
            end
        })
        ...
        Blacklist:AddDropdown('Checks/Blacklist/SelectPlayer', {
            ...
            Tooltip = 'Select Targets', -- Information shown when you hover over the dropdown
            Callback = function(Value)
                Bazed["Checks"]["Blacklist"].Usernames = Value
            end
        })
        ...
        Checks:AddToggle('Checks/Distance', {
            Text = 'Distance',
            Default = Bazed["Checks"]["Distance"].Enabled , 
            Tooltip = 'Will not lock onto people who are too far away', 
            Callback = function(Value)
                Bazed["Checks"]["Distance"].Enabled = Value
            end
        })
        ...
        Distance:AddSlider('Checks/Distance/Max', {
            Text = 'Max Distance',
            Default = Bazed["Checks"]["Distance"]["Max Distance"],
            Min = 1,
            Max = 1000,
            Rounding = 0,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Bazed["Checks"]["Distance"]["Max Distance"] = Value
            end
        })
        ...
        Resolver:AddToggle('Resolver/Enabled', {
            Text = 'Enabled',
            Default = Bazed["Resolver"].Enabled , 
            Tooltip = 'Will toggle resolver.', 
            Callback = function(Value)
                Bazed["Resolver"].Enabled = Value
            end
        }):AddKeyPicker('Resolver/ToggleBind', {
            Default = Bazed.Resolver.Keybind, 
        ...
        Resolver:AddDropdown('Resolver/Method', {
            ...
            Default = Bazed["Resolver"]["Method"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
        ...
        ssss:AddSlider('Airshot/JumpOffset', {
            Text = 'Offset',
            Default = Bazed["Airshot"]["Jump Offset"],
            Min = -5,
            Max = 5,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["Airshot"]["Jump Offset"] = Value
            end
        })
        ssss:AddDivider()
        ...
        Airshot:AddToggle('Airshot/CamlockSmoothness', {
            ...
            Default = Bazed["Airshot"]["Cam Lock Smoothness"].Enabled , 
            Tooltip = 'Will change your camlock smoothness if target is in air', 
            Callback = function(Value)
                Bazed["Airshot"]["Cam Lock Smoothness"].Enabled = Value
            end
        })
        ...
        SMoothing:AddSlider('CamLock/SmoothV2', {
            Text = 'Value',
            Default = Bazed["Airshot"]["Cam Lock Smoothness"]["New Smoothness"],
            Min = 0,
            Max = 1,
            Rounding = 10,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["Airshot"]["Cam Lock Smoothness"]["New Smoothness"] = Value
            end
        })
        ...
        Visuals:AddToggle('HitSound/Enabled', {
            Text = 'Hit Sound',
            Default = Bazed["Hit Sound"].Enabled , 
            Tooltip = 'Will play sound when you hit target', 
            Callback = function(Value)
                Bazed["Hit Sound"].Enabled = Value
            end
        })
        ...
        HitSound:AddSlider('HitSound/Volume', {
            Text = 'Volume',
            Default = Bazed["Hit Sound"].Volume * 10,
            Min = 1,
            Max = 100,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["Hit Sound"].Volume = Value / 10
            end
        })
        ...
        HitSound:AddDropdown('HitSound/Sound', {
            ...
            Default = Bazed["Hit Sound"]["Hit Sound"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Bazed["Hit Sound"]["Hit Sound"] = Value
            end
        })
        ...
        Visuals:AddToggle('HitLogs/Enabled', {
            Text = 'Hit Logs',
            Default = Bazed["Hit Logs"].Enabled , 
            Tooltip = 'Logs when you hit a player', 
            Callback = function(Value)
                Bazed["Hit Logs"].Enabled = Value
            end
        })
        ...
        Misc:AddToggle('Macro/Enabled', {
            Text = 'Macro',
            Default = Bazed["Macro"].Enabled , 
            Tooltip = 'Lua based Macro', 
            Callback = function(Value)
                Bazed["Macro"].Enabled = Value
            end
            ...
            NoUI = false, 
            ChangedCallback = function(New)
                Bazed["Macro"]["Bind"] = KeyCodeToString(New)
            end
        })
        ...
        MacroSets:AddToggle('Macro/Abuse', {
            Text = 'Abuse',
            Default = Bazed["Macro"].Abuse , 
            Tooltip = 'Use scroll wheel to macro', 
            Callback = function(Value)
                Bazed["Macro"].Abuse = Value
            end
        })

        MacroSets:AddSlider('Macro/Speed', {
            Text = 'Speed',
            Default = Bazed["Macro"].Speed,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true,
        ...
        Misc:AddToggle('MemorySpoofer/Enabled', {
            Text = 'Memory Spoofer',
            Default = Bazed["Spoofer"].Memory.Enabled , 
            Tooltip = 'Spoofs your memory', 
            Callback = function(Value)
                Bazed["Spoofer"].Memory.Enabled = Value
            end
        })
        ...
        SpooferSets:AddSlider('MemorySpoofer/Min', {
            Text = 'Minimum',
            Default = Bazed["Spoofer"].Memory.Min,
            Min = 500,
            Max = 2000,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["Spoofer"].Memory.Min = Value
            end
        })

        SpooferSets:AddSlider('MemorySpoofer/Max', {
            Text = 'Maximum',
            Default = Bazed["Spoofer"].Memory.Max,
            Min = 500,
            Max = 2000,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["Spoofer"].Memory.Max = Value
            end
        })
        ...
        Misc:AddToggle('FakeSpike/Enabled', {
            Text = 'Fake Spike',
            Default = Bazed["Fake Spike"].Enabled , 
            Tooltip = 'Spikes your ping to make it look like your lagging', 
            Callback = function(Value)
                Bazed["Fake Spike"].Enabled = Value
            end
            ...
            NoUI = false, 
            ChangedCallback = function(New)
                Bazed["Fake Spike"]["Bind"] = KeyCodeToString(New)
            end
        })
        ...
        Spike:AddSlider('FakeSpike/Duration', {
            Text = 'Duration',
            Default =  Bazed["Fake Spike"]["Duration"],
            Min = 0.1,
            Max = 3,
            Rounding = 2,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed["Fake Spike"]["Duration"] = Value
            end
        })
        ...
        Misc:AddToggle('Panic/Enabled', {
            ...
            NoUI = false, 
            ChangedCallback = function(New)
                Bazed["Panic"]["Bind"] = KeyCodeToString(New)
            end
        })
        ...
        PanicMode:AddToggle('Panic/Options', {
            Text = 'Options',
            Default = Bazed["Panic"]["Things To Disable"].Options , 
            Callback = function(Value)
                Bazed["Panic"]["Things To Disable"].Options = Value
            end
        })

        PanicMode:AddToggle('Panic/Aiming', {
            Text = 'Aiming',
            Default = Bazed["Panic"]["Things To Disable"].Aiming , 
            Callback = function(Value)
                Bazed["Panic"]["Things To Disable"].Aiming = Value
            end
        })
        ...
        PanicMode:AddToggle('Panic/FOV', {
            Text = 'FOV',
            Default = Bazed["Panic"]["Things To Disable"].FOV , 
            Callback = function(Value)
                Bazed["Panic"]["Things To Disable"].FOV = Value
            end
        })

        PanicMode:AddToggle('Panic/Misc', {
            Text = 'Misc',
            Default = Bazed["Panic"]["Things To Disable"].Misc , 
            Callback = function(Value)
                Bazed["Panic"]["Things To Disable"].Misc = Value
            end
        })
        ...
        PanicMode:AddToggle('Panic/Emotes', {
            Text = 'Emotes',
            Default = Bazed["Panic"]["Things To Disable"].Emotes , 
            Callback = function(Value)
                Bazed["Panic"]["Things To Disable"].Emotes = Value
            end
        })
        ...
        Misc:AddToggle('GunSorting/Enabled', {
            Text = 'Gun Sorting',
            Default = Bazed["Gun Sorting"].Enabled , 
            Tooltip = 'Sorts your guns in your hotbar (must set in table)', 
            Callback = function(Value)
                Bazed["Gun Sorting"].Enabled = Value
            end
            ...
            NoUI = false, 
            ChangedCallback = function(New)
                Bazed["Gun Sorting"]["Bind"] = KeyCodeToString(New)
            end
        })
        ...
        Sort:AddToggle('GunSorting/Delete', {
            Text = 'Delete Extra',
            Default = Bazed["Gun Sorting"]["Delete Extra"] , 
            Tooltip = 'Deletes unsorted items from your inventory', 
            Callback = function(Value)
                Bazed["Gun Sorting"]["Delete Extra"] = Value
            end
        })
        ...
        ESP:AddToggle('Esp/Enabled', {
            Text = 'Enabled',
            Default = Bazed["ESP"].Enabled , 
            Callback = function(Value)
                Bazed["ESP"].Enabled = Value
            end
        })

        ESP:AddSlider('ESP/MAXDISTANCE', {
            Text = 'Max Distance',
            Default = Bazed.ESP["Max Distance"],
            Min = 1,
            Max = 1000,
            Rounding = 0,
            Compact = true,
            ...
            Callback = function(Value)
                Bazed.ESP["Max Distance"] = Value
            end
        })

        ESP:AddToggle('Esp/Names', {
            Text = 'Names',
            Default = Bazed["ESP"].Name.Enabled , 
            Callback = function(Value)
                Bazed["ESP"].Name.Enabled = Value
            end
        }):AddColorPicker('NameColor', {
            Default = Bazed["ESP"].Name.Color,
            Title = 'Name Color', 
            Transparency = nil, 
        
            Callback = function(Value)
                Bazed["ESP"].Name.Color = Value
            end
        })

        ESP:AddToggle('Esp/Names/Outline', {
            Text = 'Name Outline',
            Default = Bazed["ESP"].Name.Outline , 
            Callback = function(Value)
                Bazed["ESP"].Name.Outline = Value
            end
        })
        ...
        ESP:AddToggle('Esp/Box', {
            Text = 'Box',
            Default = Bazed["ESP"].Box.Enabled , 
            Callback = function(Value)
                Bazed["ESP"].Box.Enabled = Value
            end
        }):AddColorPicker('BoxColor', {
            Default = Bazed["ESP"].Box.Color,
            Title = 'Box Color', 
            Transparency = nil, 
        
            Callback = function(Value)
                Bazed["ESP"].Box.Color = Value
            end
        })

        ESP:AddToggle('Esp/Names/Outline', {
            Text = 'Box Outline',
            Default = Bazed["ESP"].Box.Outline , 
            Callback = function(Value)
                Bazed["ESP"].Box.Outline = Value
            end
        })
        ...
        ESP:AddToggle('Esp/Health', {
            Text = 'Health Bar',
            Default = Bazed["ESP"]["Health Bar"].Enabled , 
            Callback = function(Value)
                Bazed["ESP"]["Health Bar"].Enabled = Value
            end
        }):AddColorPicker('HealthColor', {
            Default = Bazed["ESP"]["Health Bar"].Color,
            Title = 'Health Bar Color', 
            Transparency = nil, 
        
            Callback = function(Value)
                Bazed["ESP"]["Health Bar"].Color = Value
            end
        })
        ...
        ESP:AddToggle('Esp/Health/Outline', {
            ...
            Default = Bazed["ESP"]["Health Bar"].Outline , 
            Callback = function(Value)
                Bazed["ESP"]["Health Bar"].Outline = Value
            end
        })

        ESP:AddToggle('Esp/Distance', {
            Text = 'Distance',
            Default = Bazed["ESP"].Distance.Enabled , 
            Callback = function(Value)
                Bazed["ESP"].Distance.Enabled = Value
            end
        }):AddColorPicker('DistanceColor', {
            Default = Bazed["ESP"].Distance.Color,
            Title = 'Distance Color', 
            Transparency = nil, 
        
            Callback = function(Value)
                Bazed["ESP"].Distance.Color = Value
            end
        })

        ESP:AddToggle('Esp/Distance/Outline', {
            Text = 'Distance Outline',
            Default = Bazed["ESP"].Distance.Outline , 
            Callback = function(Value)
                Bazed["ESP"].Distance.Outline = Value
            end
        })
        ...
        -- UI Settings
        Library:SetWatermark('Bazed | '..tostring(os.date("%B") .. " " .. os.date("%d") .. " " .. os.date("%Y")).." | "..utility.Version)
        Library:SetWatermarkVisibility(false)
        local MenuGroup = Tabs['UI Settings']:AddLeftGroupbox('Menu')
        local MenuGroup2 = Tabs['UI Settings']:AddRightGroupbox('More Settings')
        MenuGroup:AddButton('Unload', function() Library:Unload() end)
        MenuGroup:AddLabel('Menu bind'):AddKeyPicker('MenuKeybind', { Default = Bazed["Options"]["Internal"].Bind, NoUI = true, Text = 'Menu keybind' })
        ...
        MenuGroup2:AddToggle('show_stats', {
            Text = 'Show Stats',
            Default = Bazed["Options"]["Stats"],
            Callback = function(V) 
                Bazed["Options"]["Stats"] = V
            end
        })
        
        MenuGroup2:AddToggle('allow_notifications', {
            Text = 'Allow Notifications',
            Default = Bazed["Options"]["Allow Notifications"],
            Callback = function(V) 
                Bazed["Options"]["Allow Notifications"] = V
            end
        })

        MenuGroup2:AddDropdown('notification_mode', {
            Values = NotificationTypes,
            Default = Bazed.Options["Notifcation Type"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Bazed.Options["Notifcation Type"] = Value
            end
        })
        ...
        if game:GetService("UserInputService").TouchEnabled then 
            task.spawn(function()
                local BazedMobileButton = Instance.new("ScreenGui")
                local Icon = Instance.new("Frame")
                local IconButton = Instance.new("TextButton")
                local IconImage = Instance.new("ImageLabel")
                local IconCorner = Instance.new("UICorner")
                BazedMobileButton.Name = "BazedMobileButton"
                BazedMobileButton.IgnoreGuiInset = true
                BazedMobileButton.Parent = game.CoreGui
                BazedMobileButton.DisplayOrder = 10
                BazedMobileButton.ResetOnSpawn = false
                Icon.Name = "Icon"
                Icon.Parent = BazedMobileButton
                ...
                IconImage.ZIndex = 11
                IconImage.Image = "rbxassetid://18147405186"
                ...
                IconButton.MouseButton1Down:Connect(function()
                    game:GetService("VirtualInputManager"):SendKeyEvent(true, string.upper(Bazed["Options"]["Internal UI Bind"]) , false , game)
                end)
            end)
        end
    end
